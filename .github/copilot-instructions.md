...
### Verified Action SHAs (use these exact SHAs)
...
- AWS authentication via secrets (`AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`) + role assumption (see pattern above)
...
- Reports (JaCoCo + Vitest) are served via **S3 + CloudFront with OAC**
- Terraform module path: `terraform/reports/`
- S3 bucket name: `${project_name}-reports-${environment}` (default: `naturgy-gas-reports-dev`)
...
- Outputs: `reports_bucket_name`, `reports_url` (https://domain), `cloudfront_distribution_id`
...
- **Infra workflow** (`create-reports-infra.yml`): `workflow_dispatch`, runs terraform init/plan/apply, imports existing S3 bucket if present (idempotent)
- **Deploy workflow** (`deploy-reports.yml`): `workflow_dispatch` + `workflow_run` (on CI/CD Pipeline success on main), builds backend+frontend reports, syncs to S3, invalidates CloudFront cache
...
- Backend JaCoCo reports go to `reports/jacoco`
